#!/usr/bin/env python3
"""Stage the 20 story/news Depo ads into AdMachin as full DRAFT ads (no launch).
Per ad: upload creative -> headline copy -> primary copy -> assemble draft ad, Tort / Depo Provera.
Resumable own state file (keyed by n). NO launch code.

    .venv/bin/python scripts/depo_story_admachin_stage.py [--only <n,..>]
"""
import argparse
import hashlib
import json
...
from depo_story20_gen import ADS, slug_for, OUT  # noqa: E402

PROJECT_ID = "e15c60bd-95c2-47b9-9730-c29fb5325461"      # Tort
SUBPROJECT_ID = "9cfb5b76-1dd3-4e07-b037-2dda178ac266"   # Depo Provera
STATE = pathlib.Path("outputs/depo_ads/story_admachin_state.json")
...
def stage_one(ad, st):
    key = str(ad["n"])
    rec = st.setdefault(key, {"slug": slug_for(ad)})
    img = OUT / f"{slug_for(ad)}.png"
    if not img.is_file():
        return "no-image"
    headline = ad.get("headline_fb", ad["headline"])
    primary = ad["primary"]
...
def main():
    ...
    st = load_state()
    print("Staging 20 story/news ads into Tort / Depo Provera — DRAFTS, no launch\n")
    done = 0
    for ad in ads:
        ...
        try:
            ...
            if res == "no-image":
                ...
                print(f"[ok]   {ad['n']:02d} {ad['approach'][:28]} -> {res}"); done += 1
