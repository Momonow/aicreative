"""Finalize Story A (b1, FREE google-flow path): trim -> WATERMARK-CROP -> static gain -> concat -> master.
Free tier burns a bottom-right "Veo" mark, so crop 664x1180 (center, off the bottom) and rescale to
clean 9:16 720x1280 (~8% zoom, no distortion). Output: outputs/illinois_jdc_storytime_a_b1/story_a_b1_final.mp4
"""
import json, subprocess, re, sys
from pathlib import Path

SRC = Path("outputs/illinois_jdc_storytime_a_b1")
...
def main():
    ...
    for cid in clips:
        ...
        trans = Path(f"outputs/jdc_a_b1_{cid}/transcript.json")
        if trans.exists():
    ...
    final = SRC / "story_a_b1_final.mp4"
