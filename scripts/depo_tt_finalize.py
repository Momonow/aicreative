"""Finalize + Nick-caption a depo_tt TikTok-voice ad (9:16).
Per clip: dissect -> word-aware trailing trim (intended line vs Scribe transcript, drops veo
improv on the 8s-in-short-line clips) -> per-clip loudness gain to -18 LUFS -> concat -> master
gain to -16 + true-peak limiter -> adNN_slug_final.mp4, then caption_nick.py -> adNN_slug_nick.mp4.

  python scripts/depo_tt_finalize.py 5
"""
import json
import re
import shutil
...
def bounds(line, tx):
    ...
    for w in words:
        ...
        if j < len(intended) and t == intended[j]:
            ...
            le = w["end"]
            j += 1
    return (fs, le, j, len(intended)) if fs is not None and le is not None else (None, None, j, len(intended))
...
def finalize(ad_n):
    ad = ADS[ad_n]
    adir = Path("outputs/depo_tt") / f"ad{ad_n:02d}_{ad['slug']}"
    work = adir / "fin"
    work.mkdir(exist_ok=True)
    lines = ad.get("clips")
    present = []
    print(f"== ad{ad_n:02d} {ad['slug']} ({len(lines)} clips) ==", flush=True)
    for i, line in enumerate(lines, 1):
        clip = adir / f"clip{i:02d}.mp4"
        if not clip.exists() or clip.stat().st_size < 50000:
            print(f"  clip{i:02d} MISSING — abort", flush=True)
            return None
        tx = transcript_for(clip, f"u_tt{ad_n}_c{i:02d}")
        fs, le, matched, total = bounds(line, tx)
        txt = tx.get("text", "").strip()
        tcl = work / f"t{i:02d}.mp4"
        if fs is not None and matched >= total:
            s, e = max(0.0, fs - 0.06), le + 0.14
    ...
    final = adir / f"ad{ad_n:02d}_{ad['slug']}_final.mp4"
    ...
    nick = adir / f"ad{ad_n:02d}_{ad['slug']}_nick.mp4"
    ...
    print(f"DONE ad{ad_n:02d}: {dur}s  final={final}  nick={nick}", flush=True)
    return nick


if __name__ == "__main__":
    finalize(int(sys.argv[1]) if len(sys.argv) > 1 else 5)
