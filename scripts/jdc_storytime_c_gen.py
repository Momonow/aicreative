"""Story C (Illinois juvie storytime, "The chaplain") — persona bed_b5.
...
SLUG = "illinois_jdc_storytime_c_b5"
OUT = Path(f"outputs/{SLUG}"); OUT.mkdir(parents=True, exist_ok=True)
HOST_IMG = "outputs/illinois_jdc_storytime_bed/reference/bed_b5.png"
...
CLIPS = [
    ("clip01", "A chaplain SA'ed me at Illinois juvie. This is my story.",
    ...
    ("clip03", "I was fourteen at Saint Charles. The chaplain was supposed to be the safe one. The good one.",
     "Matter-of-fact, then a flicker of bitterness.", "Faint, tired eyes.", '"Saint Charles" said plainly.'),
    ("clip04", "Kids went to his office to pray, to get a break from the unit. I went because for an hour it didn't feel like lockup.",
     "Quiet, remembering.", "Eyes drift, then settle.", PRON_PLAIN),
    ("clip05", "That office is where it happened. The one room in that whole place nobody thought to watch.",
     "Hard to say, hollow, low and steady.", "Looks down then back.", PRON_PLAIN),
    ("clip06", "Who do you even tell? The man's a chaplain. Everybody trusted him over some kid in juvie.",
     "Defeated, a quiet edge.", "Slow small head shake.", PRON_PLAIN),
    ("clip07", "So I buried it under everything else and tried to forget that room existed. For years.",
     "Vulnerable, reflective.", "Soft tired eyes.", PRON_PLAIN),
    ("clip08", "I found out later I wasn't the only kid who walked out of there carrying that. Not even close.",
