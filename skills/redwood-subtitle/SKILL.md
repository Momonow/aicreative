...
description: Burn Redwood captions with all-caps tracked Anton, white text with black stroke, and a hot-pink rounded box that karaoke-tracks the spoken word using Scribe timing. Use when the user asks for Redwood captions, pink karaoke captions, or pink highlight subtitles.
---
...
- **vpos is per-video**. Run `--preview` and show candidate positions before the full render.
...
Apply legal text only through the relevant campaign disclaimer skill.
