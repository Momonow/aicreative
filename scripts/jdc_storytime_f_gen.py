"""Story C (Illinois juvie storytime, "The nurse") — persona bed_b7.
...
SLUG = "illinois_jdc_storytime_f_b7"
OUT = Path(f"outputs/{SLUG}"); OUT.mkdir(parents=True, exist_ok=True)
HOST_IMG = "outputs/illinois_jdc_storytime_bed/reference/bed_b7.png"
...
CLIPS = [
    ("clip01", "A nurse SA'ed me at Illinois juvie. This is my story.",
     "Heavy, bracing himself; quiet and raw, the weight of saying it for the first time.", "A slow swallow before he starts.", PRON_SA),
    ("clip02", "And how I might finally be getting compensated for it. If a staff member did something to you in there, stay with me.",
     "A small turn toward hope, steadier.", "A slight lift in the eyes.", PRON_PLAIN),
    ("clip03", "I was fourteen. Caught something going around the unit, so they sent me to medical to get looked at.",
     "Matter-of-fact, quiet.", "Faint, tired eyes.", PRON_PLAIN),
    ("clip04", "Medical's supposed to be where you go when something's wrong. Where you're safe. That's where it happened to me.",
     "Quiet, the betrayal landing.", "Eyes drift, then settle.", PRON_PLAIN),
    ("clip05", "I didn't even fully understand it while it was going on. I just knew something was wrong and I froze.",
     "Hard to say, hollow, low and steady.", "Looks down then back.", PRON_PLAIN),
    ("clip06", "Who's a fourteen-year-old gonna tell? The nurse? The guards who walked me there? I kept it to myself.",
     "Defeated, a quiet edge.", "Slow small head shake.", PRON_PLAIN),
    ("clip07", "Carried it for years. Couldn't trust anybody in a white coat, couldn't let it go.",
     "Vulnerable, reflective.", "Soft tired eyes.", PRON_PLAIN),
    ("clip08", "Then I found out I wasn't close to the only one. Hundreds of us, same kind of story.",
