"""Finalize Story B (b2, FREE google-flow path): trim -> WATERMARK-CROP -> static gain -> concat -> master.
Free tier burns a bottom-right "Veo" mark, so crop 664x1180 (center, off the bottom) and rescale to
clean 9:16 720x1280 (~8% zoom, no distortion). Output: outputs/illinois_jdc_storytime_b_b2/story_b_b2_final.mp4
"""
import json, subprocess, re, sys
from pathlib import Path

SRC = Path("outputs/illinois_jdc_storytime_b_b2")
...
def main():
    ...
    for cid in clips:
        ...
        trans = Path(f"outputs/jdc_b_b2_{cid}/transcript.json")
        if trans.exists():
    ...
    final = SRC / "story_b_b2_final.mp4"
