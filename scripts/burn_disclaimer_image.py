"""Burn the verbatim Pulaski/Jones legal disclaimer onto a STATIC image ad (the image-ad
counterpart of scripts/burn_disclaimer.py). Text comes from caption_styled.DEFAULT_DISCLAIMER —
REGULATED, never paraphrased.
...
Styles:
  ...
  python scripts/burn_disclaimer_image.py <in.png> <out.png> [--style auto|band|plain]
  python scripts/burn_disclaimer_image.py <in.png> <out.png> --font-frac 0.0145
"""
import argparse
import sys
...
def burn(src, out, style="auto", font_frac=0.0145, text=None):
    ...
    max_w = int(W * 0.92)
    lines = wrap(d, text or DEFAULT_DISCLAIMER, font, max_w)
    lh = int(fpx * 1.32)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--style", choices=["auto", "band", "plain", "fit", "bar"], default="auto")
    ap.add_argument("--font-frac", type=float, default=0.0145)
    a = ap.parse_args()
    burn(a.src, a.out, a.style, a.font_frac)
