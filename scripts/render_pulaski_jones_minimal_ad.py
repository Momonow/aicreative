#!/usr/bin/env python3
"""
Render a Pulaski/Jones minimalist 4:5 text-on-black campaign ad.
...
Usage:
  .venv/bin/python scripts/render_pulaski_jones_minimal_ad.py <source.mp4> <out_dir> \\
      [--width 1080] [--height 1350] \\
      [--disclaimer-start 5.0] [--disclaimer-duration 5.0]

The script:
  1. Probes source duration
  2. Extracts mono 16k audio for ElevenLabs Scribe
...
SUBSTITUTIONS_PATH = (
    ROOT / "config/campaigns/pulaski-jones/caption_substitutions.json"
)
...
def main():
    ...
    args = ap.parse_args()
    load_substitutions(SUBSTITUTIONS_PATH)
