"""Story C (Illinois juvie storytime, "The search that wasnt a search") — persona bed_b11.
...
SLUG = "illinois_jdc_storytime_g_b11"
OUT = Path(f"outputs/{SLUG}"); OUT.mkdir(parents=True, exist_ok=True)
HOST_IMG = "outputs/illinois_jdc_storytime_bed/reference/bed_b11.png"
...
CLIPS = [
    ("clip01", "A C.O. SA'ed me at Illinois juvie. This is my story.",
     "Heavy, bracing himself; quiet and raw, the weight of saying it for the first time.", "A slow swallow before he starts.", '"C.O." = spoken letters "see-oh". ' + PRON_SA),
    ("clip02", "And how I might finally be getting compensated for it. If staff did this to you in there, stay with me.",
     "A small turn toward hope, steadier.", "A slight lift in the eyes.", PRON_PLAIN),
    ("clip03", "I was fifteen. In juvie they can search you whenever they want. They call it security.",
     "Matter-of-fact, quiet.", "Faint, tired eyes.", PRON_PLAIN),
    ("clip04", "But one officer's searches went way past a search. Every time. And he picked when nobody else was around.",
     "Quiet, the memory hardening.", "Eyes drift, then settle.", PRON_PLAIN),
    ("clip05", "He made it feel routine, like this is just how it works in here. So part of me thought maybe it was.",
     "Hollow, low and steady.", "Looks down then back.", PRON_PLAIN),
    ("clip06", "It wasn't. A search is a search. What he did was something else, and he hid it behind the rules.",
     "A quiet, certain edge.", "Slow small head shake.", PRON_PLAIN),
    ("clip07", "I never reported it. Who'd believe me over a guard. So I carried it out the gate and said nothing.",
     "Defeated, vulnerable.", "Soft tired eyes.", PRON_PLAIN),
    ("clip08", "Turns out hundreds of us have been carrying the same thing. Same facilities, same story.",
...
def main():
    ...
    print(f"[host] bed_b5 -> {host_mgid[:50]}...", flush=True)
