...
**Social-proof UI image ads (comment / chat / forum screenshots) must be DEBRANDED (2026-06/07).** The comment-thread / group-chat / forum-post family is a user-validated performer (CCWF batch 4), but every render must be a GENERIC unbranded interface: **no Facebook logo/name/blue header/like-share chrome, no Reddit logo or wordmark, no Apple/iMessage/WhatsApp/Messenger branding** — "Reddit-STYLE" layout with a made-up community label (`r/Ex_Inmates`) is fine, recognizable platform branding is not (platform-policy + trademark risk on the ad image). Bake an explicit DEBRAND clause into every prompt — see the `DEBRAND` block in `scripts/ccwf_social10.py`. Also: when extending a big image-ad campaign, keep a NO-REPEAT list of formats already run (jdcstyle20 excluded all 74 prior CCWF formats) and cap testimonial-style cards (~1 per batch) — format variety is the point.

**Campaigns that require the on-image disclaimer get it as a POST bar-burn, not in the gen prompt.** Women's-prison (Pulaski/Jones) image ads: generate the creative clean, then burn the verbatim disclaimer bar with `scripts/burn_disclaimer_image.py` at stage time, and append the same verbatim disclaimer to the FB **primary text**. Pattern: `scripts/ccwf_launch30_stage.py` (burn → upload → copy rows → assemble, resumable ONE-writer state file). Depo/IL-JDC stay disclaimer-free on the creative (`feedback_no_disclaimer_lingo_in_copy`).
